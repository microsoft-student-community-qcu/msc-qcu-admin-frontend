...
export const editApplicantSchema = z.object({
  ...
  middleInitial: z
    .string()
    .regex(/^[A-Za-z]\.?$/, { message: "Middle initial must be a single letter, optionally followed by a dot (e.g., B or B.)" })
    .optional()
    .or(z.literal("")),
  ...
  cellphone: z
    .string()
    .regex(/^09\d{9}$/, { message: "Cellphone number must be 11 digits starting with 09 (e.g., 09123456789)" }),
